...
public class URLSession {
	...
	@available(*, unavailable, message: "URLSessionConfiguration is not yet supported in WebFoundation. Use `URLSession.shared` instead.")
	...
	public func data(for request: URLRequest, delegate: Any?) async throws {
		fatalError()
	}
	...
	@available(*, unavailable, message: "Passing a delegate is not yet supported in WebFoundation")
	public func data(from url: URL, delegate: Any?) async throws -> (Data, URLResponse) {
	...
	/// - Parameter corsMode: Cross-origin mode of the request (allows CORS by default).
	/// - Returns: Data and response.
	public func upload(for request: URLRequest, from bodyData: Data, corsMode: CORSMode = .cors) async throws -> (Data, URLResponse) {
		var request = request
	...
	@available(*, unavailable, message: "Passing a delegate is not yet supported in WebFoundation")
	public func upload(for request: URLRequest, from bodyData: Data, delegate: Any?) async throws -> (Data, URLResponse) {
		fatalError()
	}
}
...
public enum CORSMode: String {
	...
	/// Only a [limited set](https://fetch.spec.whatwg.org/#concept-filtered-response-cors) of headers are exposed in the Response, but the body is readable.
	case cors
...
extension URLRequest {
	fileprivate func fetchRequest(corsMode: CORSMode) -> JSObject {
